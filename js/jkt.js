...
// LIST SONGS
const listSongs = document.querySelectorAll(".title-list-jkt p");

let songIndex = 0;
// CREATE CONTENT LOAD
window.addEventListener("DOMContentLoaded", () => {
  const item = newMusicJkt48[songIndex];
...
let show = (showMusic) => {
  const item = newMusicJkt48[showMusic];
...
nextBtn.addEventListener("click", () => {
  ...
  if (songIndex > newMusicJkt48.length - 1) {
    songIndex = 0;
  }
...
prevBtn.addEventListener("click", () => {
  ...
  if (songIndex < 0) {
    songIndex = newMusicJkt48.length - 1;
  }
