...
// LIST SONGS
const listSongs = document.querySelectorAll(".title-list-pajama p");

let songIndex = 0;
// CREATE CONTENT LOAD
window.addEventListener("DOMContentLoaded", () => {
  const item = musicPajama[songIndex];
...
let show = (showMusic) => {
  const item = musicPajama[showMusic];
...
nextBtn.addEventListener("click", () => {
  ...
  if (songIndex > musicPajama.length - 1) {
    songIndex = 0;
  }
...
prevBtn.addEventListener("click", () => {
  ...
  if (songIndex < 0) {
    songIndex = musicPajama.length - 1;
  }
